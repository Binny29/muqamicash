...
class account extends db_connect
{
    ...
    public function signup($username, $fullname, $password, $email, $country, $phone)
    {
 ...
 if (empty($country)) {
            ...
            $result = array("error" => true,
                "error_code" => ERROR_UNKNOWN,
                "error_type" => 5,
                "error_description" => "Empty country");
        ...
        $stmt = $this->db->prepare("INSERT INTO users (state, login, fullname, passw, email, salt, regtime, ip_addr, refer, country, phone) value (:state, :username, :fullname, :password, :email, :salt, :createAt, :ip_addr, :refer, :country, :phone)");
    ...
    public function restorePointCreate($email, $clientId)
    {
        ...
        $stmt = $this->db->prepare("INSERT INTO restore_data (accountId, hash, email, clientId, createAt, u_agent, ip_addr, country, phone) value (:accountId, :hash, :email, :clientId, :createAt, :u_agent, :ip_addr, :country, :phone)");
        $stmt->bindParam(":accountId", $this->id, PDO::PARAM_INT);
